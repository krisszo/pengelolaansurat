<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
                ...
                $file = $_REQUEST['file_feedback'];
                $status = $_REQUEST['status'];
                $id_user = $_SESSION['id_user'];
              

            $query = mysqli_query($config, "UPDATE tbl_surat_masuk SET status_feedback='$status', feedback ='$feedback'  WHERE id_surat='$id_surat'");
        ...
        $query = mysqli_query($config, "SELECT id_surat, no_agenda, no_surat, asal_surat, isi, kode, indeks, tgl_surat, file, keterangan, feedback, file_feedback, id_user FROM tbl_surat_masuk WHERE id_surat='$id_surat'");
        list($id_surat, $no_agenda, $no_surat, $asal_surat, $isi, $kode, $indeks, $tgl_surat, $file, $keterangan, $feedback, $file, $id_user) = mysqli_fetch_array($query);
            ...
            <div class="row jarak-form">
                ...
                <form class="col s12" method="POST" action="?page=tsm&act=tindakan" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="input-field col s6">
                            <i class="material-icons prefix md-prefix">featured_play_list</i>
                            <textarea id="isi" class="materialize-textarea validate" name="tanggapan" required readonly> <?php echo $feedback ;?>  </textarea>
                    ...
                    <div class="row">
                        ...
                        <div class="col 6">
                            <a href="?page=tsm" class="btn-large deep-orange waves-effect waves-light">Kembali <i class="material-icons">clear</i></a>
                        </div>
                    </div>
